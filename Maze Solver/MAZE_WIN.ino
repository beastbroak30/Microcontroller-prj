...
#define WHITE_CONFIRM_ATTEMPTS 4
...
int consecutiveLeftTurns = 0; // Track consecutive left turns
int consecutiveRightTurns = 0; // Track consecutive right turns
...
{   
    ...
    // Node detection with LSRB and inverse turn logic
    char movement = 'S';
    ...
    if (position > 6700)
    {
        ...
        {
            movement = 'L'; // Inverse: perform left turn
            inverseTurn = true;
            consecutiveRightTurns = 0;
            move(1, speedturn, 0);
            move(0, speedturn, 1);
    ...
    else if (position < 300)
    {
        ...
        {
            movement = 'R'; // Inverse: perform right turn
            inverseTurn = true;
            consecutiveLeftTurns = 0;
            move(1, speedturn, 1);
            move(0, speedturn, 0);
    ...
    if (isDryRun && nodeCount < MAX_NODES)
    {
        ...
        {
            // Adjust direction based on actual movement (inverse or normal)
            if (inverseTurn)
            {
                // Inverse turn: increment instead of decrement for exploration
